...
unsigned long lastSensorPublish = 10000;
...
int readLDR() {
  ...
  ldrReadings[ldrIndex] = analogRead(PIN_LDR);
  ldrTotal = ldrTotal + ldrReadings[ldrIndex];
...
// Função para publicar dados do sensor de luz
void publishLightSensorData() {
  if (millis() - lastSensorPublish >= SENSOR_PUBLISH_INTERVAL) {
    int ldrValue = readLDR();
    
    StaticJsonDocument<200> doc;
    doc["raw_value"] = ldrValue;
    doc["percentage"] = map(ldrValue, 0, 4095, 0, 100); // Converte para porcentagem
    doc["is_night_mode"] = nightMode;
    
    // Classificação do nível de luz
    String lightLevel;
    if (ldrValue < 500) {
      lightLevel = "Muito escuro";
    } else if (ldrValue < 1000) {
      lightLevel = "Escuro";
    } else if (ldrValue < 2000) {
      lightLevel = "Média luminosidade";
    } else if (ldrValue < 3000) {
      lightLevel = "Claro";
    } else {
      lightLevel = "Muito claro";
    }
    doc["light_level"] = lightLevel;
    
    char buffer[200];
    serializeJson(doc, buffer);
    client.publish(topicLightSensor, buffer);
...
void loop() {
  ...
  publishStatus();
  publishLightSensorData(); // Publica dados do sensor de luz
  delay(100);
}
